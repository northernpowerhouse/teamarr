...
class ConditionEvaluator:
    ...
    def _eval_home_win_streak(
        ...
        if not value or not game_ctx.streaks:
            return False
        try:
            streak = game_ctx.streaks.home_streak
            if not streak or not streak.startswith("W"):
                return False
            streak_count = int(streak[1:])
            return streak_count >= int(value)
    ...
    def _eval_away_win_streak(
        ...
        if not value or not game_ctx.streaks:
            return False
        try:
            streak = game_ctx.streaks.away_streak
            if not streak or not streak.startswith("W"):
                return False
            streak_count = int(streak[1:])
            return streak_count >= int(value)
    ...
    def _eval_is_playoff(
        ...
        if not event:
            return False
        # Check season_type field: 3 = postseason
        return event.season_type == "3" or event.season_type == 3
    ...
    def _eval_is_preseason(
        ...
        if not event:
            return False
        # Check season_type field: 1 = preseason
        return event.season_type == "1" or event.season_type == 1
    ...
    def _eval_is_national_broadcast(
        ...
        # Our Event stores broadcasts as list of strings
    ...
    def _eval_has_odds(
        ...
        return game_ctx.odds is not None and game_ctx.odds.spread > 0
...
def select_template(
    ...
    For repeated selection, use get_condition_selector() directly.
    """
    return get_condition_selector().select(description_options, ctx, game_ctx)
